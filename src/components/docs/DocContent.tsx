...
import type { Member } from './TeamMembers'
...
export default function DocContent({ page }: { page: DocPageData }) {
  ...
  // 挂载文档"岛"：视频(ArtPlayer) / 团队成员卡 / 贡献者表 / 版本号 span（懒加载、按需）
  useEffect(() => {
    const host = contentRef.current
    if (!host) return
    const en = lang === 'en'

    // 版本号：<span data-version="iso|hub"> 从 Cloud-PE API 填充（纯 DOM）
    const versionEls = host.querySelectorAll<HTMLElement>('[data-version]')
    if (versionEls.length) {
      const loading = en ? 'Loading…' : '加载中…'
      const failed = en ? 'Failed' : '获取失败'
      versionEls.forEach((e) => {
        e.textContent = loading
      })
      void fetch('https://api.cloud-pe.cn/v2/data.json')
        .then((r) => (r.ok ? r.json() : Promise.reject(new Error('bad'))))
        .then((data) => {
          const iso = data?.data?.cloud_pe_version
          const hub = data?.cloud_pe_one?.version
          versionEls.forEach((e) => {
            const k = e.getAttribute('data-version')
            if (k === 'iso') e.textContent = iso ? `Cloud-PE ${iso}` : failed
            else if (k === 'hub') e.textContent = hub ? `Cloud-PE One ${hub}` : failed
          })
        })
        .catch(() => {
          versionEls.forEach((e) => {
    ...
    // React 岛
    const videoEls = host.querySelectorAll<HTMLElement>('[data-player="video"]')
    const teamEls = host.querySelectorAll<HTMLElement>('[data-team]')
    const contribEls = host.querySelectorAll<HTMLElement>('[data-contributors]')
    if (!videoEls.length && !teamEls.length && !contribEls.length) return
    ...
    void Promise.all([
      ...
      import('./TeamMembers'),
      import('./Contributors'),
    ]).then(([client, videoMod, teamMod, contribMod]) => {
      if (cancelled) return
      const { createRoot } = client
      const VideoPlayer = videoMod.default
      const TeamMembers = teamMod.default
      const Contributors = contribMod.default
      const mount = (el: HTMLElement, node: React.ReactElement) => {
        el.innerHTML = ''
      ...
      teamEls.forEach((el) => {
        let members: Member[] = []
        try {
          members = JSON.parse(el.getAttribute('data-members') ?? '[]')
        } catch {
          members = []
        }
        mount(el, <TeamMembers members={members} />)
      })
      contribEls.forEach((el) => mount(el, <Contributors en={en} />))
    })
    return () => {
      cancelled = true
  ...
  // 否则挂载在正文里的「视频/团队卡/贡献者表」等 React 岛会被一并清空消失。
